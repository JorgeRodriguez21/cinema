...
internal class MovieReviewRequestTest {
    ...
    fun `should throw an exception when the rate is less than 1`() {
        val actualException =
            assertThrows(IllegalArgumentException::class.java) { MovieReviewRequest(1, "", 0, "username") }
    ...
    fun `should throw an exception when the rate is more than 5`() {
        val actualException =
            assertThrows(IllegalArgumentException::class.java) { MovieReviewRequest(1, "", 6, "username") }
    ...
    fun `should not throw an exception when the rate is between 1 and 5`() {
        val rate = (1..5).random()
        assertDoesNotThrow { MovieReviewRequest(1, "", rate, "username") }
    }
}